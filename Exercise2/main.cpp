...
// *argv = array di puntatori a caratteri che contiene gli argomenti passati da riga di comando. argv[0] è il nome del programma, argv[1] è il primo argomento, argv[2] è il secondo argomento e così via.
{
    ...
    {      
        ...
        std::chrono::steady_clock::time_point start_time = std::chrono::steady_clock::now();  //prendo il tempo di inizio dell'ordinamento
        SortLibrary::BubbleSort<int>(v); //chiamo la funzione di ordinamento BubbleSort per ordinare il vettore v (copia di v1)
        std::chrono::steady_clock::time_point end_time = std::chrono::steady_clock::now(); //prendo il tempo di fine dell'ordinamento
        time_elapsed_bubble_v1 += std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count(); //calcolo il tempo di esecuzione dell'ordinamento in microsecondi e lo sommo al tempo totale di esecuzione
    }
    ...
    {  
        ...
        std::chrono::steady_clock::time_point end_time = std::chrono::steady_clock::now(); //prendo il tempo di fine dell'ordinamento
        time_elapsed_bubble_v2 += std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count(); //calcolo il tempo di esecuzione dell'ordinamento in microsecondi e lo sommo al tempo totale di esecuzione
    }
    ...
    {      
        ...
        SortLibrary::BubbleSort<int>(v); //chiamo la funzione di ordinamento BubbleSort per ordinare il vettore v (copia di v1)
        std::chrono::steady_clock::time_point end_time = std::chrono::steady_clock::now(); //prendo il tempo di fine dell'ordinamento
        time_elapsed_bubble_v3 += std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count(); //calcolo il tempo di esecuzione dell'ordinamento in microsecondi e lo sommo al tempo totale di esecuzione
    }
